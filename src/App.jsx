...
function App() {
  ...
  const filterNotesByTag = (tagNum) => {
    if (tagNum == "0" || !tagNum) return getLocalNotes(sort);

    return getLocalNotes(sort).filter((note) => note.tag === tagNum);
  };
  ...
  const changeTag = (e) => {
    ...
    setNotesData(sortNotes(filterNotesByTag(tagNum), sort, sortDirection));
  };
  ...
  const [tags, setTags] = useState(() => {
    ...
    setNotesData(sortNotes(filterNotesByTag(tag), sort, sortDirection));
      ...
      <div className="flex flex-col gap-3 w-2xs sm:w-xl md:w-2xl">
        ...
        <div className="flex justify-between gap-8 sm:gap-4 items-end sm:grid sm:grid-cols-3">
          <div className="absolute top-2 left-2 sm:relative sm:top-0 sm:left-0 sm:justify-self-start">
            <Button className={["bg-primary-bg", "hover:bg-secondary", "border-2", "border-primary", "w-10", "h-8", "rounded-[50%]", "flex", "justify-center", "items-center", "sm:w-15", "sm:h-10"]}>
        ...
        <div className="rounded-2xl py-2 px-2 sm:py-5 sm:px-5 bg-secondary-bg flex flex-col gap-2">
          {notesData.length > 0 ? (
            notesData.map((note) => {
              return (
                <Note sortNum={sort} onClick={editNote} bgColor={`bg-note-${note.priority}`} checked={note.checked} dateChange={note.dateChange} dateCreate={note.dateCreate} key={note.noteId} id={note.noteId}>
                  {note.name}{" "}
                </Note>
              );
            })
          ) : (
            <div className="text-base sm:text-lg text-center">Добавьте первую заметку</div>
          )}
        </div>
      </div>
